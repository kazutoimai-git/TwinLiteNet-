...
class TwinLiteNetPlus(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.up_1_prompt = UpConvBlock(model_cfg['chanels'][1], model_cfg['chanels'][0]) # H/8 -> H/4
        self.up_2_prompt = UpConvBlock(model_cfg['chanels'][0], 8)                       # H/4 -> H/2
        self.out_prompt = UpConvBlock(8, 1, last=True) # 最終出力チャネルを1 (ヒートマップ) に変更
    ...
    def forward(self, input):
        ...
        out_prompt = self.up_1_prompt(out_caam, inp2)
        out_prompt = self.up_2_prompt(out_prompt, inp1)
        out_prompt = self.out_prompt(out_prompt)
        
        return out_da,out_ll,out_prompt
